...
class SelectorsVmTest < Minitest::Test
  ...
  def test_filter_by_template_true_alias
    selector = Pvectl::Selectors::Vm.parse("template=true")
    result = selector.apply(@all_vms)
    assert_equal 1, result.size
    assert_equal 9000, result.first.vmid
  end
  ...
  def test_filter_by_template_false_alias
    selector = Pvectl::Selectors::Vm.parse("template=false")
    result = selector.apply(@all_vms)
    assert_equal 4, result.size
  end

  def test_filter_by_template_1_alias
    selector = Pvectl::Selectors::Vm.parse("template=1")
    result = selector.apply(@all_vms)
    assert_equal 1, result.size
    assert_equal 9000, result.first.vmid
  end
  ...
  def test_filter_by_template_0_alias
    selector = Pvectl::Selectors::Vm.parse("template=0")
    result = selector.apply(@all_vms)
    assert_equal 4, result.size
  end
